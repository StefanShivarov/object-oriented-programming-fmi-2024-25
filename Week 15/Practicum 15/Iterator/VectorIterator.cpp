...
public:
    ...
    public:
        ...
        offset operator-(const Iterator& other) const {
            return current - other.current;
        }
        Iterator& operator+=(offset n) {
            current += n;
            return*this;
        }
        Iterator operator+(offset n) {
        ...
        bool operator<(const Iterator& o) const noexcept { return current < o.current; }
        bool operator>(const Iterator& o) const noexcept { return current > o.current; }
        bool operator<=(const Iterator& o) const noexcept { return current <= o.current; }
        bool operator>=(const Iterator& o) const noexcept { return current >= o.current; }
        T& operator[](offset n) const noexcept {
            return *(current + n);
        }
    ...
    RIterator rbegin() {
        return RIterator(data+currentSize-1);
    }
    RIterator rend() {
